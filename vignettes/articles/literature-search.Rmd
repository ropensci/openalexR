...
With the **rentrez** package, we can use the `entrez_search` function, which retrieves up to 10 records matching the search query from the PubMed database.
...
We can limit the number of results with the paging options `per_page` (number of records to return per page, between 1 and 200, default 200) and `pages` (range of pages to return, *e.g.*, `1:3` for the first 3 pages, default NULL to return all pages), supplied via `options = oa_options(...)`.
